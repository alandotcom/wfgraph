...
const CLIENT_ENTRYPOINT = "./src/client/index.html";
...
async function setupWatchers(): Promise<FSWatcher[]> {
  ...
  const recursiveWatcher = createWatcher("./src", true);
  if (recursiveWatcher) {
    watchers.push(recursiveWatcher);
    logInfo("watching ./src recursively");
  } else {
    const directories = await collectDirectories("./src");
