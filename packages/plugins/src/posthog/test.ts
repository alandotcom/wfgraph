...
export async function testPostHog(
  ...
  // An OAuth grant stores the project's `api_token`, which is still a `phc_`
  // key, but the prefix rule is asked of a key the operator typed alone. Which
  // one this is comes from the caller rather than from the token's own text.
  const isOAuthProjectKey = context.oauthCredentialKeys.includes(
    "POSTHOG_PROJECT_API_KEY"
  );

  if (!isOAuthProjectKey && !projectApiKey.startsWith("phc_")) {
    return {
      success: false,
      error:
