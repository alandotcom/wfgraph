...
/** A key the stored PostHog OAuth grant issued. */
const granted = { oauthCredentialKeys: ["POSTHOG_PROJECT_API_KEY"] };
...
describe("testPostHog", () => {
  it("names the credential when there is none", async () => {
    expect(await testPostHog({}, manual)).toEqual({
      success: false,
  ...
  it("names the prefix before spending a request", async () => {
    ...
    expect(
      await testPostHog({ POSTHOG_PROJECT_API_KEY: "phx_personal" }, manual)
    ).toEqual({
      success: false,
      error:
  ...
  it("skips the prefix rule for a granted project key and validates it normally", async () => {
    stubFetch(() =>
    ...
    expect(
      await testPostHog(
        { POSTHOG_PROJECT_API_KEY: "granted-project-token" },
        granted
      )
  ...
  it("accepts a key the flags endpoint evaluates", async () => {
    ...
    expect(
      await testPostHog({ POSTHOG_PROJECT_API_KEY: "phc_good" }, manual)
    ).toEqual({
      success: true,
    });
  });
  ...
  it("reports PostHog's own detail when PostHog is what refused", async () => {
    ...
    expect(
      await testPostHog({ POSTHOG_PROJECT_API_KEY: "phc_bad" }, manual)
    ).toEqual({
      success: false,
  ...
  it("reports a bare status when the refusal is not PostHog's", async () => {
    ...
    expect(
      await testPostHog({ POSTHOG_PROJECT_API_KEY: "phc_x" }, manual)
    ).toEqual({
      success: false,
