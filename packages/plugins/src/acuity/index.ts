...
export function createAcuity(createSdk: CreateAcuitySdk) {
  return defineIntegration({
    ...
    test: async () => {
      const { testAcuity } = await import("#src/acuity/test");
      return (creds) => testAcuity(creds, createSdk);
    },
    ...
    actions: {
      "list-appointment-types": {
        ...
        handler: Effect.fn(function* (bag) {
          const client = yield* createAcuityClient(bag, createSdk);
      ...
      "cancel-appointment": {
        ...
        handler: Effect.fn(function* (bag) {
          ...
          const noShow = yield* optionalBoolean(
            input.noShow,
            "Mark as No-Show"
          );
          const admin = yield* optionalBoolean(input.admin, "Run as Admin");
...
export const acuity = createAcuity(createAcuitySdk);
