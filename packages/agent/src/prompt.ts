...
- A Group is an organizational frame around steps the user drew on the canvas.
  ...
  connect_nodes names the steps inside it. add_node creates a step in no Group.
  insert_node_on_edge puts the new step in a Group when both ends of the edge
  are in that Group. When delete_node leaves a Group with fewer than two steps,
  the Group is removed and its remaining step stays in the graph. An Event Split
  inside a Group does not count toward that minimum. When a tool result says a
  Group was removed, name that Group in your reply.
