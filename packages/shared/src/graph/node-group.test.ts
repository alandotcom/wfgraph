...
describe("groupCanvasPositions", () => {
  ...
  describe("branches converging on a join", () => {
    ...
    it.each(cases)(
      "$name in a $direction Group: each join follows its predecessors and no cards overlap",
    ...
    it("stands a member with one predecessor in that predecessor's column", () => {
      const { memberIds, interiorEdges } = shape("a>b", "b>c", "x>y");
      for (const direction of ["vertical", "horizontal"] as const) {
        const positions = groupCanvasPositions({
          memberIds,
          interiorEdges,
        ...
        const across = (id: string) =>
          direction === "vertical"
            ? positions.get(id)?.x
            : positions.get(id)?.y;
        expect(across("y")).toBe(across("x"));
        expect(across("c")).toBe(across("b"));
      }
    });
  });
