...
describe("the pools an app opens", () => {
  ...
  it("hand the discrete fields to postgres.js as fields", () => {
    ...
    expect(options.user).toBe("wfgraph");
    expect(options.database).toBe("wfgraph_config_test");
  });
  ...
  it("carry a database name a URL would have to escape", () => {
    expect(
      open({ ...DISCRETE_CONFIG, database: "wfgraph test" }).client.options
        .database
    ).toBe("wfgraph test");
  });
...
describe("opening a second surface", () => {
  it("keeps each app's database handle on its own surface", () => {
    ...
    expect(first.client.options.database).toBe("wfgraph_config_test");
