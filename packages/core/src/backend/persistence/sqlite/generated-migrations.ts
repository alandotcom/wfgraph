...
export const sqliteMigrations = [
  {
    sql: [
      ...
      "\nCREATE UNIQUE INDEX `workflow_executions_workflow_id_delivery_id_uidx` ON `workflow_executions` (`workflow_id`,`delivery_id`);",
      ...
      "\nCREATE UNIQUE INDEX `workflow_versions_workflow_id_version_uidx` ON `workflow_versions` (`workflow_id`,`version`);",
  ...
  {
    sql: [
      "ALTER TABLE `workflow_executions` ADD `entity_type` text;",
      "\nALTER TABLE `workflow_executions` ADD `entity_id` text;",
      "\nALTER TABLE `workflow_executions` ADD `termination_kind` text;",
      "\nALTER TABLE `workflow_executions` ADD `termination_requested_at` integer;",
      "\nALTER TABLE `workflow_executions` ADD `termination_reason` text;",
      "\nALTER TABLE `workflow_executions` ADD `termination_node_id` text;",
      ...
      "\nINSERT INTO `__new_workflow_executions`(`id`, `workflow_id`, `workflow_version_id`, `workflow_run_id`, `status`, `start_source`, `delivery_id`, `enqueued_at`, `run_mode`, `start_event_name`, `entity_value`, `input`, `output`, `error`, `started_at`, `waiting_at`, `cancelled_at`, `completed_at`, `duration`, `cancel_requested_at`, `cancel_event_name`, `cancel_payload`) SELECT `id`, `workflow_id`, `workflow_version_id`, `workflow_run_id`, `status`, `start_source`, `delivery_id`, `enqueued_at`, `run_mode`, `start_event_name`, `entity_value`, `input`, `output`, `error`, `started_at`, `waiting_at`, `cancelled_at`, `completed_at`, `duration`, `cancel_requested_at`, `cancel_event_name`, `cancel_payload` FROM `workflow_executions`;",
      "\nDROP TABLE `workflow_executions`;",
      "\nALTER TABLE `__new_workflow_executions` RENAME TO `workflow_executions`;",
      "\nPRAGMA foreign_keys=ON;",
      "\nCREATE UNIQUE INDEX `workflow_executions_workflow_id_delivery_id_uidx` ON `workflow_executions` (`workflow_id`,`delivery_id`);",
      "\nCREATE INDEX `executions_workflow_started_idx` ON `workflow_executions` (`workflow_id`,`started_at` DESC,`id` DESC);",
      "\nCREATE INDEX `executions_started_idx` ON `workflow_executions` (`started_at` DESC,`id` DESC);",
      "\nCREATE INDEX `executions_entity_idx` ON `workflow_executions` (`workflow_id`,`entity_value`,`run_mode`,`status`);",
      "\nCREATE INDEX `executions_typed_entity_idx` ON `workflow_executions` (`workflow_id`,`entity_type`,`entity_id`,`run_mode`,`status`);",
      "\nCREATE INDEX `executions_workflow_in_flight_version_started_idx` ON `workflow_executions` (`workflow_id`,`workflow_version_id`,`started_at`) WHERE \"workflow_executions\".\"status\" in ('pending', 'running', 'waiting');",
    ],
    bps: true,
    folderMillis: 1789042199000,
    hash: "344ec5fb8a7995651c92943da5d5eb622f944190dc5ae0b03227edc2e9b83d12",
    name: "20260910120959_peaceful_gladiator",
  },
  {
    sql: [
    ...
    bps: true,
    folderMillis: 1789042224000,
    hash: "a2e8ff78f994dcf85db0816c6a2a557ce8be125f19823866d1991e9b7831b0b9",
    name: "20260910121024_backfill-termination-claims",
  },
  {
    sql: [
      "ALTER TABLE `workflow_executions` DROP COLUMN `cancel_requested_at`;",
    ],
    bps: true,
    folderMillis: 1789042307000,
    hash: "3f2eb6ae0ec5434c94ebf2067aa01d767c5b8a4b9c0487035ddfc05940d72457",
    name: "20260910121147_warm_gargoyle",
  },
