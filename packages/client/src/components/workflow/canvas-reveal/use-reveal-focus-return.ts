 ...
 * opener, or to `fallbackTarget` once the opener has left the document. Every
 * move passes `preventScroll`: Reveal may still be sliding in when its title
 * takes focus, and a scrolling focus would drag the editor shell after it.
 */
  ...
  useBeforePaint(
    ...
    () => {
      ...
      if (level === "focus" && shown.level !== "focus") {
        input.title.current?.focus({ preventScroll: true });
      }
      if (level === "browse" && shown.level === "focus") {
        input.focusToggle.current?.focus({ preventScroll: true });
      }
