...
describe("revealWidth", () => {
  it.each([
    { canvas: 900, browse: 320, focus: 900 - 2 * REVEAL_INSET },
    ...
    ({ canvas, browse, focus }) => {
      expect(revealWidth("closed", canvas)).toBe(0);
      expect(revealWidth("browse", canvas)).toBe(browse);
      expect(revealWidth("focus", canvas)).toBe(focus);
    }
  );

  it("leaves Focus 256 px of canvas where its step would not fit", () => {
    expect(revealWidth("focus", 1024)).toBe(640);
    expect(revealWidth("focus", 1290)).toBe(720);
    expect(revealWidth("focus", 1536)).toBe(800);
    expect(revealWidth("focus", 1030)).toBe(640);
    expect(revealWidth("focus", 1000)).toBe(1000 - 2 * REVEAL_INSET);
  });

  it("stays within a canvas narrower than Browse", () => {
    expect(revealWidth("browse", 300)).toBe(300 - 2 * REVEAL_INSET);
  });
});
