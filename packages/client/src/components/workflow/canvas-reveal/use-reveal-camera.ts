...
import { setRevealCameraAtom } from "#src/lib/workflow-workspace-navigation";
import {
  revealViewport,
  sameWorldCamera,
 ...
 * level or subject, closes, or answers a placement request, so the subject
 * stays visible beside it. It only calls `setViewport`: no layout runs and no
 * node moves. Closing restores the camera from before Reveal unless a person
 * moved the viewport in between. Nothing is compared while the canvas cannot
 * be placed, so a change made then is acted on once it can.
...
export function useRevealCamera(input: {
  ...
  useAfterPaint(slotKey, () => {
    ...
    const stored = state.presentation.revealCamera;
    const animate = (camera: WorldCamera) => {
      inFlightRef.current = camera;
      void setViewport(viewportFromWorldCamera(camera, size), {
        duration: viewportAnimationDuration(),
      });
    };

    if (step === "restore") {
      store.set(setRevealCameraAtom, {
        address: state.address,
        revealCamera: null,
      });
      const restore = untouchedBefore(current, stored);
      if (restore && !sameWorldCamera(restore, current)) {
        animate(restore);
      }
      return;
    }
    ...
    const before = untouchedBefore(current, stored) ?? current;
    ...
    const target = usable
      ? worldCameraFromViewport(
          revealViewport({
            viewport: viewportFromWorldCamera(before, size),
            usable,
            bounds,
          }),
          size
    ...
    store.set(setRevealCameraAtom, {
      address: state.address,
      revealCamera: { before, placed: target },
    });
