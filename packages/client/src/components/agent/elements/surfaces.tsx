/**
 * The surface and label primitives assistant-ui's step panel is built from,
 * vendored from `r.assistant-ui.com/base/elements-surfaces.json`.
 *
 * Library source, edited where this repo's own imports differ and in
 * `SwapLabel`, whose two refs are declared separately so the React Compiler
 * this app builds with cannot memoise them into one array and lose the hook
 * order, and whose inactive layer now leaves the accessibility tree rather than
 * carrying `aria-hidden="false"` (both layers were being read, so a trigger
 * announced "ThinkingDone thinking"). The `mono` step comes onto this system's
 * type scale, and the dead `live` constant is deleted: it named a raw Tailwind
 * palette class, which DESIGN.md calls a defect. It ships its whole surface rather than only the names the panel reads,
 * so a later re-vendor is a copy rather than a merge; `knip.ts` exempts the
...
export function SwapLabel({
  ...
  // Two separate calls rather than `[useRef(), useRef()]`. This app builds with
  // the React Compiler, which memoises an array literal and can then skip the
  // hook calls inside it on a later render, changing the hook order and
  // throwing "Should have a queue" out of this component.
    ...
    >
      {children.map((layer, index) => (
        <span
          key={index}
          ref={layers[index]}
          aria-hidden={active !== index || undefined}
          className={cn(
            labelSwap,
