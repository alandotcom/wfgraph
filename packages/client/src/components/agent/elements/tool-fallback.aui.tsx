/**
 * assistant-ui's tool-call row, vendored from the Base UI registry at
 * `r.assistant-ui.com/base/tool-fallback.json`.
 *
 * One local edit beyond the imports: every trigger and its icons are set at
 * `text-xs` / `size-3.5`. The registry sizes each element for a full-page chat
 * and does not size them alike, which in a panel beside the canvas reads as
 * three type sizes stacked in one column.
 *
 * Its trigger takes `isError`, because assistant-ui marks a tool call complete
 * the moment a result arrives whether or not the tool refused, so a trigger
 * reading only `status` gave every refusal a checkmark. It also drops the
 * registry's "Used tool:" prefix and the bold around the name: `agent-thread.tsx` passes a sentence saying what the call did, and
 * the prefix restates what the row already is. `ToolFallbackResult` treats a
 * null result as nothing to print, for the same reason.
 */
...
function ToolFallbackTrigger({
  ...
  // A refused call carries `isError` and a settled status, because assistant-ui
  // marks a tool call complete the moment a result arrives whether or not the
  // tool refused. Reading the status alone would give every refusal a check.
  const Icon = isError ? XCircleIcon : statusIconMap[statusType];

  return (
    <CollapsibleTrigger
      data-slot="tool-fallback-trigger"
      className={cn(
        "aui-tool-fallback-trigger group/trigger text-muted-foreground hover:text-foreground flex w-fit origin-left items-center gap-2 py-1.5 text-xs transition-[color,scale] active:scale-[0.98] motion-reduce:transition-none motion-reduce:active:scale-100",
        className
      )}
      {...props}
    >
...
function ToolFallbackResult({
  ...
  // `null` as well as `undefined`: a read tool answers no sentence, and its
  // call carries a null result purely to mark itself finished. Printing
  // "Result: null" under the row would be the opposite of what that means.
