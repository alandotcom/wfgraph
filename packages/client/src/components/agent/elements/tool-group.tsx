/**
 * assistant-ui's ToolGroup disclosure, vendored from the Base UI registry at
 * `r.assistant-ui.com/base/tool-group.json`.
 *
 ...
 *
 * Library source otherwise, edited to drop the deprecated `ToolGroup` wrapper the registry ships for the older
 * `components.ToolGroup` prop, which nothing here uses. `agent-thread.tsx`
 * composes Root, Trigger and Content itself.
 */
...
function ToolGroupTrigger({
  ...
  return (
    <CollapsibleTrigger
      ...
      className={cn(
        "aui-tool-group-trigger group/trigger flex origin-left items-center gap-2 py-1.5 text-xs transition-[color,scale] active:scale-[0.98] motion-reduce:transition-none motion-reduce:active:scale-100",
    ...
    >
      {active && (
        <LoaderIcon
          ...
          className="aui-tool-group-trigger-loader size-3.5 shrink-0 animate-spin [animation-duration:0.6s] motion-reduce:animate-none"
        />
      )}
      <span
