/**
 * The list helpers assistant-ui's step panel reads, vendored from
 * `r.assistant-ui.com/base/elements-range.json`.
 *
 * Library source, edited only where this repo's own imports differ. It ships
 * its whole surface rather than only the names the panel reads, so a later
 * re-vendor is a copy rather than a merge; `knip.ts` exempts the directory for
 * that reason.
 */
