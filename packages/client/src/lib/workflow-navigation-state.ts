...
/**
 * The cameras around one automatic Reveal placement: `before` is the camera the
 * placement was computed from, and `placed` is the camera it moved to. A
 * viewport still at `placed` has not been moved by a person since.
 */
export type RevealCamera = { before: WorldCamera; placed: WorldCamera };
...
export type DesktopScopePresentation = ScopePresentation & {
  ...
  revealCamera: RevealCamera | null;
};
...
export function withRevealCamera(
  scope: ScopeNavigation,
  revealCamera: RevealCamera | null
): ScopeNavigation {
  return scope.desktop.revealCamera === revealCamera
    ? scope
    : { ...scope, desktop: { ...scope.desktop, revealCamera } };
}
