...
The workflow overview draws each Group as one collapsed card: solid Graphite Wash behind a 1.5px Canvas Line border, a title band holding the Group icon, the label, the issue badge, and an **Enter group** arrow, and below a rule the number of steps, or on a run's canvas the Group's run status and step counts. The Wash is what separates a Group from a step, and it inverts on its own in dark. Its members never expand in place. Edges entering the Group attach to the card's one top handle, and edges leaving it attach to its one bottom handle, which carries no label. Every edge from inside the Group to the same outside step draws as one edge from that handle, and when every edge it stands for leaves by the same Condition branch, it carries that branch's True or False label. While nothing leaves the Group, dragging from the bottom handle to an outside step adds a connection from every place a path ends inside the Group, each step that connects to nothing and each unconnected Condition outlet, so that step runs once after every branch finishes; a join the rules refuse, such as both branches of one Condition, is refused with its notice. Once the Group continues, dragging from the bottom handle adds a connection from the outlets it already continues by. Deleting an edge drawn from the card deletes every connection it stands for, and an outlet entering the Group at several steps draws one edge onto the card, whose deletion deletes the connection to each of those steps. Continuing one branch alone is done on the focused Group canvas.

Two gestures add a step where a path already runs, and they read the same on the workflow overview and on a focused Group canvas. **Add step after** on a step's context menu, one row per outlet so a Condition, an Event Split and the Lifecycle Node say which branch, adds a step on that outlet: the step runs beside whatever the outlet already reaches and takes an edge to each of those next steps, so the branch rejoins them and a Group keeps its exits. A rejoin the connection rules refuse is left out and said in a notice, and the step stays. Dragging a connection from an outlet into empty canvas adds the same step, and **Add Step** from the palette or the toolbar does too while one step is selected. **Insert step**, on a connection's context menu and on the + control the connection draws while it is hovered, selected, or focused, puts the step inside that connection instead: the connection's source reaches the new step and the new step reaches what the connection reached. On a collapsed Group card, or a Group's "Continues to" stub, the connection stands for every stored edge it draws, and all of them are replaced. Each gesture is one undo step and goes through the Group rules. **Ungroup** shows on the menu of a Group card and a Group member, and nowhere else.

The focused Group canvas shows only the Group's members, as standard cards laid out in rows from the Group's interior edges, with those edges between them. A step with one source stands in its source's column, a step where branches join sits in the row after its last branch, near the middle of the steps that feed it, and the steps of one row keep the order of the steps that feed them, so each step's children stand together. An edge turns across in the gap just before the step it reaches. An edge that passes rows runs straight down its source's column when no card and no other edge in those rows is in its way; otherwise it turns in the gap after its source's row into its own lane beside the cards, runs along that lane past those rows, and turns into its target's column in the gap before it, so no edge runs through or along a card. Edges turning in one gap take separate tracks across it wherever their runs across the flow would touch, so no two edges draw over each other. Edges leaving the same outlet share a track and draw as one line that forks, edges reaching the same step or stub share one and draw as lines that join once, and each stretch they share is drawn once so its dashes stay even. A gap needing more tracks than its height holds grows taller. Each Group stores a layout direction, **Top to bottom** or **Left to right**. Top to bottom stacks the rows downward with handles on the cards' top and bottom; Left to right runs the rows across with handles on the cards' left and right. The layout depends on the Group's steps, edges and direction alone, so moving the collapsed card never moves a member or the camera a person left on the Group. The Group card itself is not drawn. Each outside step that enters the Group is a dashed stub before the members reading "Incoming from" and the step's name, one per outlet it enters by, so a Condition entering by both branches draws two stubs with True and False on their edges. Each step the Group continues to is a dashed stub after them reading "Continues to" and the step's name. Each place a path ends inside the Group is a dashed stub on the same line reading "Path ends", with an edge from a step that connects to nothing, or from a Condition's unconnected True or False outlet, which then carries that branch label. A stub one outlet reaches stands level with that outlet, and a "Continues to" stub several outlets reach stands centred across them, the way an "Incoming from" stub stands centred over the steps it enters, or as near that centre as the stubs beside it leave room for. When there is no room between those stubs, every stub on the line spreads apart in order across the flow, True before False, so no edge to a stub crosses another. An outlet that enters the Group at several steps draws one stub with an edge to each of them. An edge between two members can be selected and deleted, and dragging from one member's outlet to another member adds one. An edge from an "Incoming from" stub can be selected and deleted on its own, and dragging from that stub onto another member adds one more edge from the same outside outlet. An edge to a "Continues to" stub can be selected and deleted on its own, and dragging from a member's outlet onto that stub connects that outlet alone to the outside step it names. A Group is entered from one outside outlet, so a connection that would enter it from a second outlet is refused with a notice. A connection that would continue the Group to a second outside step from several outlets, give a step inside the Group a branch from outside it, or put a Condition on a branch into a join, is refused with the notice Publish would give. The focused canvas offers the same authoring as the overview for the Group's own steps: selecting and editing a step in Reveal, **Add Step** from the palette, the toolbar and the canvas menu, dragging a connection from a member or an "Incoming from" stub into empty canvas, **Paste**, **Duplicate**, disabling, deleting, and connecting. A step added, pasted, or duplicated there becomes a member of the Group, and a step added by dragging out of a member, by **Add step after**, or by **Insert step** connects in the same undo step. Every such change is one undo step, and a change the Group rules refuse stores nothing and shows the refusal as a notice. Copying a step there copies that step alone. Stubs cannot be selected or dragged, "Path ends" stubs and their edges cannot be selected or connected, and members cannot be dragged. The Group places its steps from its connections and direction, so no action there writes a step coordinate, and **Tidy layout** is disabled with the reason "Layout is automatic inside a Group". **Ungroup** places the steps in the same rows along the Group's direction, centred on the collapsed card, with the first row where the card was, then moves those steps together down and to the right until no card on the overview overlaps them. A bar at the canvas's top left holds a **Workflow** button that returns to the overview, the workflow and Group names, and the step count.
