...
Workflow Graph publishes no server. `createWfGraphApp` returns a fetch handler and the host mounts it,
...
deciding which of Workflow Graph and Vite saw a request first, a second copy of the SPA-path rule,
...
Workflow Graph on `node:http` through `createRequestListener`. Nothing from the old root server
...
- **Keep the integrated dev server** rejected: it is code no adopter runs, so the effort
  ...
  served it through Workflow Graph's, so a routing question had two answers. The stage-1 review's
...
- The `isSpaPath` rule in `packages/core/src/backend/lib/http/client-assets.ts` has one
  ...
  outside Workflow Graph applies the rule and the two modes cannot disagree about it.
