...
what is remembered. Workflow Graph declared it for them, in a place they could not see,
...
**Workflow Graph namespaces the id.** An author writes `"post"` and the runtime sees
...
At-most-once per node moves from Workflow Graph to the author. A handler that wraps nothing
