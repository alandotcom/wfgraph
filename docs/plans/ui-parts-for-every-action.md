...
`packages/client/src/main.tsx:8` imports `integrationUi` from `@rova/plugins/ui` and
passes it to the provider at line 110. The import is fixed at the SPA's build. A host
hands `createRovaApp` a built bundle, so a host that writes an icon component has no way
...
**4. Give the record a second writer.** `RovaClientBundle` in `packages/core/src/app.ts`
...
- Rova exposes a global the host's own script fills before the bundle runs. This costs
...
`useIntegrationUi` becomes `useExtensionUi`. The type moves out of `@rova/plugins/ui`,
which a host must not have to depend on, into `@rova/shared`. `@rova/plugins/ui` keeps
...
2. **Vocabulary.** Rename the type and the hook, move the type to `@rova/shared`.
...
- `logoUrl` still works. It stays the cheap path for an action that wants an image and
  no component.
- The server bundle still carries no React. Only `@rova/plugins/ui` and the new host
  record hold components, and neither is reachable from `@rova/plugins`.
